...
/生图 竖图 一只戴宇航头盔的橘猫
/生图 横图：雨后的城市夜景
/生图 --size=1024x1536 一座空中花园
/生图 尺寸 1536x1024 海边日落
/生图人物
/生图状态
```
...
尺寸未写时使用“生成参数 → 默认生图尺寸”。新安装默认使用 `816x816`；已有安装
若配置中已保存旧默认值，可在插件设置中手动选择“小方图 816×816（较快）”。
单次指令支持 `小图`、`小方图`、`小竖图`、`小横图`、`方图`、`竖图`、`横图`、
`auto` 以及对应的精确尺寸。小尺寸分别为 `816x816`、`640x1024`、`1024x640`。
仅能使用设置中“允许使用的尺寸”清单里的值，确认消息与 `/生图状态` 会显示实际尺寸。
